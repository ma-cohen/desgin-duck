...
describe("init", () => {
  ...
  test("creates requirements/ directory", () => {
    init(testDir);
    expect(existsSync(join(testDir, "requirements"))).toBe(true);
  });
  ...
  test("project.yaml contains name and description fields", () => {
    init(testDir);
    const content = readFileSync(join(testDir, "requirements", "project.yaml"), "utf-8");
  ...
  test("main.yaml contains empty requirements array", () => {
    init(testDir);
    const content = readFileSync(join(testDir, "requirements", "main.yaml"), "utf-8");
    expect(content).toContain("requirements: []");
  });

  test("derived.yaml contains empty requirements array", () => {
    init(testDir);
    const content = readFileSync(join(testDir, "requirements", "derived.yaml"), "utf-8");
    expect(content).toContain("requirements: []");
  });

  test("aborts with exit code 1 if requirements/ already exists", () => {
    mkdirSync(join(testDir, "requirements"));
    init(testDir);
    expect(process.exitCode).toBe(1);
  });

  test("does not create files when requirements/ already exists", () => {
    mkdirSync(join(testDir, "requirements"));
    init(testDir);
    expect(existsSync(join(testDir, "requirements", "project.yaml"))).toBe(false);
  });
